...
| **DOL LCA Enrichment** (`app/lca_enrichment.py`) | Real historical H-1B/H-1B1/E-3 sponsorship data from DOL/OFLC's public quarterly LCA disclosure files — a distinct signal from the Visa Scanner's posting-text-only check. Manual by design: DOL's site blocks unattended downloads, so a human downloads the quarterly `.xlsx` and runs `python -m app.cli lca-enrich <path>`. Matches by company name, pushes onto existing Beacon/Job Log rows the same way FMP/StartupHub enrichment does. |
